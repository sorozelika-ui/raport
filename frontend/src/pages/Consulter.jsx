...
// import TelephoneAppel from "./TelephoneAppel";
...
//             }}
//           >
//             <Search size={20} style={{ marginRight: "8px", color:"green"}} />
//             Rechercher
//           </Button>
//         </div>
//       </Panel>
...
//   <div>
//     <h3>{selectedPrestataireData.nom}</h3>
//     <p>📧 {selectedPrestataireData.email}</p>
//   </div>

//   <TelephoneAppel telephone={selectedPrestataireData.telephone} />
...
const Consulter = () => {
  ...
  const loadPrestataires = async () => {
    ...
    try {
      const res = await axios.get("http://127.0.0.1:8000/api/prestataire");
      const prestData = Array.isArray(res.data) ? res.data : res.data.data || [];
      setPrestataires(
        prestData.map((p) => ({
          label: `${p.nom} - ${p.specialite}`,
          value: p.id,
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (res.data && (res.data.evaluations?.length > 0 || res.data.length > 0)) {
        setResultats(res.data);
        toaster.push(
          <Message type="success" showIcon>
            Résultats trouvés !
  ...
  const groupedByYear =
    evaluationsData.length > 0
      ? evaluationsData.reduce((acc, evalItem) => {
          const year = evalItem.annee?.annee || "N/A";
          acc[year] = acc[year] || [];
          acc[year].push(evalItem);
          return acc;
        }, {})
      : {};
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "20px" }}>
      ...
      <Panel bordered style={{ marginBottom: "30px", padding: "30px" }}>
        <div style={{ display: "flex", gap: "15px", alignItems: "flex-end" }}>
          <div style={{ flex: 1 }}>
            <label
              style={{
      ...
      {/* Infos prestataire + téléphone */}
      <AnimatePresence>
        {selectedPrestataireData && resultats && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
          >
            ...
            >
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: "200px" }}>
                  <h2 style={{ margin: 0 }}>{selectedPrestataireData.nom}</h2>
                  {selectedPrestataireData.email && (
                    <p>📧 {selectedPrestataireData.email}</p>
                  )}
                </div>
            ...
            {/* Ici tu peux ajouter le rendu des évaluations comme avant */}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
