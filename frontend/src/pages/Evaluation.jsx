...
//         <Form fluid>
//           <Form.Group>
//             <Form.ControlLabel style={{ fontWeight: "600", color: "black" }}>
//               Prestataire
//             </Form.ControlLabel>
//             <SelectPicker
//               data={prestataires}
...
//             <Panel
//               bordered
//               style={{ background: "#f0f9ff", marginBottom: "15px" }}
//             >
...
//             }
//             bordered
//             style={{ marginTop: "20px", background: "#fafafa" }}
//           >
...
//               >
//                 <p>
//                   Cliquez sur "Ajouter un critère" pour commencer l'évaluation
//                 </p>
//                 <p style={{ fontSize: "14px" }}>
//                   Vous devez évaluer sur exactement 5 critères
//                 </p>
//               </div>
//             ) : (
...
//                     marginBottom: "15px",
//                     background: "white",
//                     borderLeft: "4px solid rgba(59, 130, 246, 0.2)",
//                   }}
//                 >
//                   <div
//                     style={{
...
//                       <Tag
//                         color={notesParCritere[index] >= 10 ? "green" : "red"}
//                         size="lg"
//                       >
...
//                             Justification obligatoire (note {"<"} 5)
//                           </span>
//                         </div>
//                         <Input
//                           as="textarea"
...
//                 <Panel
//                   bordered
//                   style={{ background: "#e0f2fe", marginTop: "20px" }}
//                 >
//                   <div style={{ textAlign: "center" }}>
//                     <h3
//                       style={{ margin: 0, color: "#1e40af", fontSize: "32px" }}
//                     >
//                       Moyenne : {moyenne}/20
//                     </h3>
//                     <p style={{ margin: "10px 0 0", color: "#6b7280" }}>
//                       Calculée sur {Object.keys(notesParCritere).length}{" "}
//                       critère(s)
//                     </p>
//                   </div>
//                 </Panel>
...
//                 />
//                 <h2 style={{ margin: 0, color: "#1e40af", fontSize: "48px" }}>
//                   {resultat.moyenne}/20
//                 </h2>
//                 <p style={{ margin: "10px 0", fontSize: "18px" }}>
//                   Moyenne Générale
//                 </p>
//                 <Tag size="lg" color={resultat.moyenne >= 10 ? "green" : "red"}>
...
//                   </HeaderCell>
//                   <Cell>
//                     {(row) => row.annee?.liban || row.annee?.annee || "N/A"}
//                   </Cell>
//                 </Column>
//               </Table>
...
const Evaluation = () => {
  ...
  // Cache des évaluations existantes pour la vérification doublon
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const evalData    = initial.evaluations    ?? initial.data?.evaluations    ?? [];
      ...
      // Stocker les évaluations existantes pour la vérification doublon
      setEvaluationsExistantes(Array.isArray(evalData) ? evalData : []);
  ...
  const dejaEvalue = (prestataireId, anneeId) => {
    ...
    return evaluationsExistantes.some(
      (e) =>
        (e.type_prestataire_id === prestataireId || e.prestataire?.id === prestataireId) &&
        (e.annees_id === anneeId || e.annee?.id === anneeId)
    );
  };

  // ── Handlers sélection avec avertissement immédiat ──
  const handlePrestataireChange = (value) => {
    setSelectedPrestataire(value);
    if (value && selectedAnnee && dejaEvalue(value, selectedAnnee)) {
      const nom = prestataires.find((p) => p.value === value)?.nom || "Ce prestataire";
  ...
  const handleAnneeChange = (value) => {
    ...
    if (value && selectedPrestataire && dejaEvalue(selectedPrestataire, value)) {
      const nom = prestataires.find((p) => p.value === selectedPrestataire)?.nom || "Ce prestataire";
  ...
  const handleSubmit = async () => {
    ...
    // ── Blocage doublon (dernière vérification avant envoi) ──
    if (dejaEvalue(selectedPrestataire, selectedAnnee)) {
      const nom = prestataires.find((p) => p.value === selectedPrestataire)?.nom || "Ce prestataire";
      const anneeLabel = annees.find((a) => a.value === selectedAnnee)?.label || "cette année";
      toaster.push(
    ...
    try {
      ...
      // Mettre à jour le cache local pour bloquer immédiatement une re-soumission
      setEvaluationsExistantes((prev) => [
        ...prev,
        { type_prestataire_id: selectedPrestataire, annees_id: selectedAnnee },
      ]);
  ...
  const selectedPrestataireData = prestataires.find((p) => p.value === selectedPrestataire);
  const moyenne = calculerMoyenne();
  const doublon = dejaEvalue(selectedPrestataire, selectedAnnee);
        ...
        <Form fluid>
          ...
          >
              ...
              selectedCriteres.map((item, index) => (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
                    <h5 style={{ margin: 0, color: "black" }}>Critère {index + 1}</h5>
                    <Button appearance="ghost" color="red" size="sm" onClick={() => handleRemoveCritere(index)}>
                      Supprimer
                    </Button>
                  </div>
            ...
            {selectedCriteres.length > 0 && Object.keys(notesParCritere).length > 0 && (
              <Panel bordered style={{ background: "#e0f2fe", marginTop: "20px" }}>
                <div style={{ textAlign: "center" }}>
                  <h3 style={{ margin: 0, color: "#1e40af", fontSize: "32px" }}>
                    Moyenne : {moyenne}/20
                  </h3>
                  <p style={{ margin: "10px 0 0", color: "#6b7280" }}>
                    Calculée sur {Object.keys(notesParCritere).length} critère(s)
                  </p>
                </div>
              </Panel>
            )}
          ...
          <Form.Group style={{ marginTop: "30px" }}>
            <Button
              ...
              onClick={handleSubmit}
              loading={submitting}
              disabled={submitting || selectedCriteres.length !== 5 || doublon}
              size="lg"
              block
              style={{
                background: doublon
                ...
                fontSize: "16px",
                fontWeight: "600",
                cursor: doublon ? "not-allowed" : "pointer",
              }}
            >
              {submitting
                ? "Enregistrement..."
          ...
          {resultat && (
            <div>
              ...
              <Table height={300} data={resultat.evaluations || []} bordered cellBordered>
                <Column width={200} flexGrow={1}>
                  <HeaderCell style={{ background: "rgba(59, 130, 246, 0.2)", color: "black" }}>Prestataire</HeaderCell>
                  <Cell>{(row) => row.prestataire?.nom || "N/A"}</Cell>
                </Column>
                <Column width={150}>
                  <HeaderCell style={{ background: "rgba(59, 130, 246, 0.2)", color: "black" }}>Spécialité</HeaderCell>
                  <Cell>{(row) => row.prestataire?.specialite || "N/A"}</Cell>
                </Column>
                <Column width={200} flexGrow={1}>
                  <HeaderCell style={{ background: "rgba(59, 130, 246, 0.2)", color: "black" }}>Critère</HeaderCell>
                  <Cell>{(row) => row.critere?.libcrit || "N/A"}</Cell>
                </Column>
                <Column width={100} align="center">
                  <HeaderCell style={{ background: "rgba(59, 130, 246, 0.2)", color: "black" }}>Note</HeaderCell>
                  <Cell>
                    {(row) => (
                      <Tag color={row.note?.nts >= 10 ? "green" : "red"}>
                        {row.note?.nts || 0}/20
                ...
                <Column width={120} align="center">
                  <HeaderCell style={{ background: "rgba(59, 130, 246, 0.2)", color: "black" }}>Année</HeaderCell>
                  <Cell>{(row) => row.annee?.liban || row.annee?.annee || "N/A"}</Cell>
                </Column>
              </Table>

              <Panel bordered style={{ marginTop: "20px", textAlign: "center" }}>
                <Button appearance="primary" onClick={() => setShowResult(false)}>
                  Fermer
                </Button>
              </Panel>
            </div>
